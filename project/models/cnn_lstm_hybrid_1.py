...
class CNNLSTMHybrid(nn.Module):
    def __init__(self, num_numerical_features, categorical_vocab_sizes, categorical_embedding_dims, hidden_size, num_layers, dropout):
        ...
        cnn_kernel_size = 3
        # Using a single 1D convolution to extract local features
        ...
        self.conv_layer = nn.Sequential(
            nn.Conv1d(
                ...
                padding=cnn_kernel_size // 2  # Padding to preserve sequence length
            ),
            nn.ReLU(),
            nn.Dropout(dropout)
        )
        
        # The LSTM input dimension will now be 'hidden_size' (from the CNN output channels)
        self.lstm = nn.LSTM(
    ...
    def forward(self, x_num, x_cat, lengths):
        ...
        # For CNN, transpose to the tensor shape to: [batch, channels, seq_len], to match the Conv1d expected input shape
        x_cnn = x_combined.transpose(1, 2) # shape: [batch, combined_input_dim, max_seq_length]
        
        # Apply 1D CNN, then activation and dropout
        cnn_out = self.conv_layer(x_cnn) # shape: [batch, hidden_size, max_seq_length]
